...
interface TaskAutomation {
  ...
  task_categories: any; // Use any to handle Supabase's join structure
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Get all task automations with automation enabled
    ...
    for (const automation of automations) {
      try {
        // Get the hs_queue_id from the joined task_categories (handle array structure)
        const taskCategory = Array.isArray(automation.task_categories) 
          ? automation.task_categories[0] 
          : automation.task_categories;
        const hs_queue_id = taskCategory?.hs_queue_id;
        if (!hs_queue_id) {
          console.warn(`[${executionId}] ⚠️ Skipping automation ${automation.id} - no hs_queue_id found`);
          continue;
        }
        
        console.log(`[${executionId}] 📄 Processing automation ${automation.id} for list ${automation.hs_list_id}...`);
        ...
        do {
          ...
          console.log(`[${executionId}] 🌐 Fetching page ${page} for list ${automation.hs_list_id}...`);
        ...
        console.log(`[${executionId}] 🎯 Total members fetched for list ${automation.hs_list_id}: ${allMembers.length}`);
