-- Update get_enriched_events function to include automation_id
...
BEGIN
  ...
  -- Then return the paginated results with total count including automation_id
  RETURN QUERY
  SELECT 
    e.id,
    e.created_at,
  ...
  GROUP BY e.id, e.created_at, e.event, e.type, e.hs_contact_id, 
           ...
           e.hs_engagement_id, e.hubspot_url, e.automation_id, e.logs
  ORDER BY 
