-- Fix get_owner_tasks function to properly filter tasks by owner
...
BEGIN
    ...
    WHERE (
        ...
        (et.hubspot_owner_id = owner_id_param AND et.hubspot_owner_id = ANY(valid_owners) AND et.status = 'not_started')
        OR
        -- Owner's completed tasks for today only
        (et.hubspot_owner_id = owner_id_param AND et.hubspot_owner_id = ANY(valid_owners) AND et.status = 'completed' AND DATE(et.completion_date AT TIME ZONE 'Europe/Paris') = paris_today)
        OR
        ...
        (et.hubspot_owner_id = owner_id_param AND et.hubspot_owner_id = ANY(valid_owners) AND et.status = 'not_started' AND et.raw_due_date < paris_now)
    )
    ORDER BY 
