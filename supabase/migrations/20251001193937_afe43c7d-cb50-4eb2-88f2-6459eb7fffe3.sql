-- Update handle_task_completion_automation to require associated_contact_id
...
BEGIN
  ...
  -- Check if associated_contact_id is not null
  IF NEW.associated_contact_id IS NULL THEN
    RAISE NOTICE 'Task % has no associated contact', NEW.hs_object_id;
    RETURN NEW;
  END IF;
