...
DROP FUNCTION IF EXISTS public.get_task_categories(text);
DROP FUNCTION IF EXISTS public.get_all_tasks();
DROP FUNCTION IF EXISTS public.get_owner_tasks(text);

-- Recreate the get_task_categories function to include order_by_position_in_sequence
CREATE OR REPLACE FUNCTION public.get_task_categories(team_id_param text DEFAULT NULL::text)
 RETURNS TABLE(id bigint, label text, color text, hs_queue_id text, order_column integer, locks_lower_categories boolean, task_display_order text, order_by_position_in_sequence boolean)
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
BEGIN
  RETURN QUERY
  SELECT tc.id, tc.label, tc.color, tc.hs_queue_id, tc.order_column, tc.locks_lower_categories, tc.task_display_order, tc.order_by_position_in_sequence
  ...
  WHERE 
    ...
    (
      tc.hs_queue_id IS NOT NULL AND (
        ...
        (
          -- Categories with NULL visible_team_ids are visible to everyone
          tc.visible_team_ids IS NULL
          OR
          -- Categories where the team_id is explicitly in the visible_team_ids array
          -- Empty arrays [] mean "visible to no one"
...
-- Recreate the get_all_tasks function to include number_in_sequence and created_by_automation_id
...
-- Recreate the get_owner_tasks function to include number_in_sequence and created_by_automation_id
