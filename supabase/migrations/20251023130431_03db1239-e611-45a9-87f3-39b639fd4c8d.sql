-- Update get_enriched_events function to support multiple event types
DROP FUNCTION IF EXISTS public.get_enriched_events(text, text, bigint[], text[], text[], text, text, integer, integer);
...
BEGIN
  ...
  WHERE 
    ...
    AND (
      ...
      (update_status_filter = 'tasks_updated' AND 
       ...
       (e.logs->'task_updates'->'summary'->>'total_update_successful')::int > 0) OR
  ...
  GROUP BY e.id, e.created_at, e.event, e.type, e.hs_contact_id, 
           c.firstname, c.lastname, e.hs_owner_id, u.firstname, u.lastname,
